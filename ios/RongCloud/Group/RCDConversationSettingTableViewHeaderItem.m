  ...
  if ([userModel.portraitUri isEqualToString:@""]) {
    ...
    self.ivAva.image = portrait;
      ...
      dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          NSData* imagedata=[NSData dataWithContentsOfURL:[NSURL URLWithString:userModel.portraitUri]];
          UIImage *portraitimage=[UIImage imageWithData:imagedata];
          //待子线程里面的图片加载完成，回到主线程更新界面
          dispatch_async(dispatch_get_main_queue(), ^{
              if (portraitimage) {
                  self.ivAva.image=portraitimage;
              }else{
                  self.ivAva.image = [UIImage imageNamed:@"icon_person"];
              }
          });
      });
  }
